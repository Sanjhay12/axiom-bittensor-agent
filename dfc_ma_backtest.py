...
def run_tactical_backtest(vals, short_p = SHORT_MA, long_p = LONG_MA, buy_amt = BUY_AMT, sell_frac = SELL_FRAC, min_profit = 0.0, fee = FEE, max_mult = 3.0):
    n = len(vals)
    cash = 0
    tao = 0
    invested = 0
    ...
    for i in range(long_p, n):
        ...
        if price < m_short and (price > m_long or long_rising):
              dip_frac = (m_short - price) / m_short
              mult = min(1 + dip_frac * 10, max_mult)  # bigger dip below short MA -> bigger buy, capped
              this_buy = buy_amt * mult
              new_tao = (this_buy * (1 - fee)) / price
              if tao + new_tao > 0:
...
def run_core_backtest(vals, long_p = LONG_MA, buy_amt = 1.0, fee = FEE, margin = 0.0):
    n = len(vals)
    cash = 0.0
    tao = 0.0
    invested = 0.0
    ...
    for i in range(long_p, n):
        ...
        if not in_position and price > m_long * (1 + margin):
              ...
              in_position = True
              entries += 1
        elif in_position and price < m_long * (1 - margin):
              # exit fully to cash
